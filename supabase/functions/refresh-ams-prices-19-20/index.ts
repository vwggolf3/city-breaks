...
serve(async (req) => {
  ...
  try {
    console.log('🔄 Starting Amsterdam prices batch refresh (weekends 19-20)...');
    ...
    console.log(`📅 Generated ${weekendCombinations.length} weekend combinations (weekends 19-20 × 3 types)`);
    ...
    return new Response(
      JSON.stringify({
        ...
        message: `Processed ${totalProcessed} destinations with ${weekendCombinations.length} weekend combinations (weekends 19-20). ${Math.max(0, remainingDestinations)} destinations remaining.`,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error) {
    console.error('❌ Error in refresh-ams-prices-19-20 function:', error);
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Unknown error',
        details: 'Failed to refresh Amsterdam flight prices for weekends 19-20'
      }),
